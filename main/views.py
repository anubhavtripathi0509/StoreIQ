...
from .models import Banner,Category,Brand,Product,ProductAttribute,Cart,CartItems,CartOrder,CartOrderItems,ProductReview,Wishlist,UserAddressBook
...
def checkout(request):
	...
	if 'cartdata' in request.session:
		...
		payment = client.order.create({'amount':rs, 'currency':'INR', 'payment_capture':1})
		...
		# phone=UserAddressBook.objects.filter(user=request.user,status=True).second()
